          ...
          >
            ...
            <form onSubmit={stage === 'login' ? handleLogin : handleSignup} className="w-full">
              {stage === 'signup' && (
                <InputField 
                  ref={nameRef}
                  icon={User} type="text" placeholder={t('name_placeholder')} label={t('name_label')} 
                />
              )}
              
              <InputField 
                ref={emailRef}
                icon={Mail} type="email" placeholder="email@address.com" label={t('email_label')} 
              />
              
              <InputField 
                ref={passwordRef}
                icon={Lock} type="password" placeholder="••••••••" label={t('password_label')} 
              />
              
              {stage === 'signup' && (
                <InputField 
                  ref={confirmPasswordRef}
                  icon={Lock} type="password" placeholder="••••••••" label={t('confirm_password_label')} 
                />
              )}
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {stage === 'login' ? t('btn_authenticate') : t('btn_complete')}
            ...
            <div className="mt-8 flex flex-col items-center">
              <div className="flex items-center gap-3 w-full mb-6">
                ...
                <span className="text-[9px] tracking-widest text-gray-600 uppercase font-bold">{t('sns_access')}</span>
                <div className="flex-1 h-px bg-gold-500/10"></div>
              </div>
                ...
                >
                  <div className="w-14 h-14 rounded-full border border-gold-500/10 bg-white/5 flex items-center justify-center group-hover:border-gold-500/40 transition-all shadow-xl">
                    <svg className="w-6 h-6" viewBox="0 0 24 24">
                      ...
                      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-1.01.67-2.28 1.07-3.71 1.07-2.87 0-5.3-1.94-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              ...
              >
                {stage === 'login' ? (
                  <>{t('no_account')} <span className="text-gold-500 font-bold ml-1 border-b border-gold-500/30 pb-0.5">{t('apply_membership')}</span></>
                ) : (
                  <>{t('already_member')} <span className="text-gold-500 font-bold ml-1 border-b border-gold-500/30 pb-0.5">{t('secure_login')}</span></>
                )}
              </button>
            </div>
          </motion.div>
